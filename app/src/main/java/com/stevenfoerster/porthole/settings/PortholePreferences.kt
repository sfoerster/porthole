 ...
 */
@Singleton
class PortholePreferences @Inject constructor(
    @ApplicationContext private val context: Context,
) {
    private val dataStore = context.dataStore

    /** Emits the current [SessionConfig] whenever any preference changes. */
    val sessionConfig: Flow<SessionConfig> = dataStore.data.map { prefs ->
        SessionConfig(
    ...
    val connectivityCheckUrl: Flow<String> = dataStore.data.map { prefs ->
